...
def multiscale_supervision(gt_occ, ratio, gt_shape):
    ...
    '''
    # gt_shape:[bs, cls, w, h, z]
    gt = torch.zeros([gt_shape[0], gt_shape[2], gt_shape[3], gt_shape[4]]).to(gt_occ.device).type(torch.float)  # [bs, w, h, z]
    gt_occ = gt_occ.to(torch.float)
    for i in range(gt.shape[0]):
        coords = gt_occ[i][:, :3].type(torch.long) // ratio     # scaled coords:[]
