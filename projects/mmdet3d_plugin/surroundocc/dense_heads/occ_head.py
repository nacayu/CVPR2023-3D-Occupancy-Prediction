...
class OccHead(nn.Module): 
    ...
    def _init_layers(self):
        ...
        for i in range(self.fpn_level):     # generate empty volumes
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        volume_embed = []
        for i in range(self.fpn_level):     # multi-image feature level
            ...
            view_features = self.transfer_conv[i](mlvl_feats[i].reshape(bs*num_cam, C, H, W)).reshape(bs, num_cam, -1, H, W)    # [bs, cam, c, h, w]
    ...
    def loss(self,
        ...
        if not self.use_semantic:   # default settings
            loss_dict = {}
            ...
            # flatten gcc to surroundocc format: [bs, 4]
            gcc_flatten = []
            gt_occ_np = gt_occ.cpu().numpy()
            for i in range(gt_occ.shape[0]):  
                gcc_per_batch = []
                for x in range(gt_occ.shape[1]):
                    for y in range(gt_occ.shape[2]):
                        for z in range(gt_occ.shape[3]):
                            gcc_per_batch.append([x, y, z, gt_occ_np[i, x, y, z]])
                gcc_flatten.append(gcc_per_batch)      # flatten bevformer gt_gcc to surroundocc format
            
            # convert to surroundocc labels
            gcc_flatten_tensor = torch.from_numpy(np.array(gcc_flatten)).to(gt_occ.device)
            gcc_flatten_tensor = gcc_flatten_tensor[gcc_flatten_tensor[..., 3] < 17].unsqueeze(0)   # get unfree voxels, 17 in bevformer is free
            gcc_flatten_tensor[..., 3] += 1     # remap [0, 16] to [1, 17], free voxels are defined as 0
            
            for i in range(len(preds_dicts['occ_preds'])):
                pred = preds_dicts['occ_preds'][i]
                ratio = 2**(len(preds_dicts['occ_preds']) - 1 - i)
                gt = multiscale_supervision(gcc_flatten_tensor.clone(), ratio, preds_dicts['occ_preds'][i].shape)
